...
Editor shortcuts follow desktop conventions: use Ctrl/Cmd+Z to undo, Ctrl+Y or Cmd+Shift+Z to redo, and Ctrl/Cmd+C, X, and V to copy, cut, and paste a pixel selection. Arrow keys nudge a selection by one pixel or pan the canvas when no selection is active. On desktop, scroll or right-drag to pan, pinch or Ctrl-scroll to zoom, and right-click or two-finger tap for canvas actions. On mobile, two fingers always pan and pinch-zoom regardless of the active tool.
