...
The whole surface costs **1,949 tokens** of context, because every tool's description and schema sits in the model's context on every turn. That budget is the reason for most of the design below.
...
Sending a full 32×32 sprite as `[{x, y, color}, …]` costs about 18,000 tokens. As rows of palette characters it costs about 690 — twenty six times less — and the agent can see what it is writing:
...
Two more entrypoints live in DNS rather than in this repo:

```dns
; ARD — points registries at the capability manifest
_catalog._agents.mcpixels.app. 300 IN TXT "url=https://mcpixels.app/.well-known/ai-catalog.json"

; DNS-AID — ServiceMode HTTPS record for the agent entrypoint
_index._agents.mcpixels.app.   300 IN HTTPS 1 mcpixels.app. alpn="h2,http/1.1" port=443
```

The skills index is regenerated by the first step of `npm run build`, so editing
a SKILL.md updates its `sha256` digest without a separate step.
...
by Google; the glyphs inlined in `src/components/icons.tsx` keep that license.
